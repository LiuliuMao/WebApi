...
namespace Common.Helper
{
    ...
    {
        private static string AesKey { get; set; } = "liumaosoft";
        ...
        /// <returns></returns>
        public static string AES(string strContent, string strKey = "liumaosoft")
        {
            if (string.IsNullOrEmpty(strContent)) return string.Empty;
            if (strKey.Length > 8) strKey = strKey.Substring(0, 8);
        ...
        /// <param name="strKey">加密时使用的键值 Unicode编码 必须大于或等于8个英文或汉字</param>
        /// <returns></returns>
        public static string AESDecrypt(string strContent, string strKey = "liumaosoft")
        {
            if (string.IsNullOrEmpty(strContent)) return string.Empty;
            if (strKey.Length > 8) strKey = strKey.Substring(0, 8);
            //与加密时Base64对应
