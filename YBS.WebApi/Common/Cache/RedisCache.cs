...
namespace Common.Cache
{
    public class RedisCache : IRedisCache
    {
        ...
        public RedisCache(IOptionsMonitor<AppSettings> optionsMonitor)
        {
            _optionsMonitor = optionsMonitor.CurrentValue;
            if (!string.IsNullOrWhiteSpace(_optionsMonitor.RedisConnectionString))
            {
        ...
        private ConnectionMultiplexer GetRedisConnection()
        {
            ...
            lock (redisConnectionLock)
            {
                ...
                try
                {
                    redisConnection = ConnectionMultiplexer.Connect(_optionsMonitor.RedisConnectionString);
                }
                catch (Exception)
                {
